...
public class MemberTeamAPIController {
    ...
    //다중 조회용
    @GetMapping("/memberteams/teamsearch/{teamIds}")
    public Res findAllTeamMember(@PathVariable("teamIds") List<Long> teamIds) {
        Map<String, List<Res.TeamDto>> enterMember = memberTeamService.findAllMemberTeam(teamIds);
        return Res.isOk(enterMember);
    }
